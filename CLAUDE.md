...
- Auth: Microsoft Sign-In via MSAL.js redirect flow (single allowed account: `fullnelsongrip@gmail.com`) with self-signed JWT. No login page — nav Sign In button redirects straight to Microsoft. AuthContext handles MSAL redirect response on any route. Backend verifies Microsoft ID tokens via JWKS. Microsoft Client ID read from shared plain App Config key (`microsoft_oauth_client_id_plain`). Public view-only access for browsing plants, photos, events, tasks, and analyses. Login required for management actions (CRUD, logging events, chat, capture, analysis triggers)
